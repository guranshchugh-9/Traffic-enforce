...
def cross_mask_iou_distance(atracks, btracks):
    amasks = np.array([track.mask for track in atracks]).astype(np.uint8)
    bmasks = np.array([track.cross_mask for track in btracks]).astype(np.uint8)

    intersection = (amasks.reshape(len(amasks), 1, -1) & bmasks.reshape(1, len(bmasks), -1)).sum(-1)
    union = (amasks.reshape(len(amasks), 1, -1) | bmasks.reshape(1, len(bmasks), -1)).sum(-1)

    return np.float32(intersection) / np.float32(union)
...
def get_dist_mat(v1, v2):
        '''
        v1 is a vector of dimenstion (M, D)
        v2 is a vector of dimension (N, D)
        '''
        bigv1 = np.repeat(v1, v2.shape[0], axis = 0)
        bigv2 = np.tile(v2, (v1.shape[0], 1))
        dist_mat = np.sqrt(np.sum((bigv1 - bigv2) ** 2, axis=-1))
        dist_matrix = np.reshape(dist_mat, (v1.shape[0], v2.shape[0]))
        return dist_matrix
