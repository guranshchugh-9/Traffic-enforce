...
class Mosaic(BaseMixTransform):
    ...
    def _cat_labels(self, mosaic_labels):
        ...
        for labels in mosaic_labels:
            ...
            assoc_ids.append(labels['assoc_ids'])
        ...
        final_labels = {
            ...
            'assoc_ids' : np.concatenate(assoc_ids, 0),
            'instances': Instances.concatenate(instances, axis=0),
            'mosaic_border': self.border}  # final_labels
        final_labels['instances'].clip(imgsz, imgsz)
        good = final_labels['instances'].remove_zero_area_boxes()
        final_labels['cls'] = final_labels['cls'][good]
        final_labels['assoc_ids'] = final_labels['assoc_ids'][good]
        return final_labels
...
class MixUp(BaseMixTransform):
    ...
    def _mix_transform(self, labels):
        ...
        labels['assoc_ids'] = np.concatenate([labels['assoc_ids'], labels2['assoc_ids']], 0)
        return labels
...
class RandomPerspective:
    ...
    def __call__(self, labels):
        ...
        # if(len(cls[i]) == 0):
        #     assoc_ids = np.zeros(0)
        labels['cls'] = cls[i]
        labels['assoc_ids'] = assoc_ids[i]
        labels['img'] = img
    ...
    def __call__(self, labels):
        ...
        if self.p and len(instances.segments):
            ...
            for j in random.sample(list(indexes), k=round(self.p * n)):
                ...
                assoc_ids = np.concatenate((assoc_ids, assoc_ids[[j]]), axis=0)
...
class Format:
    ...
    def __call__(self, labels):
        ...
        cls = labels.pop('cls')
        instances = labels.pop('instances')
        assoc_ids = labels.pop('assoc_ids')
        ...
        if self.return_mask:
            if nl:
                masks, instances, cls, assoc_ids = self._format_segments(instances, cls, w, h, assoc_ids)
        ...
        labels['assoc_ids'] = torch.from_numpy(assoc_ids) if nl else torch.zeros(nl)
    ...
    def _format_segments(self, instances, cls, w, h, assoc_ids=None):
        ...
        if self.mask_overlap:
            ...
            cls = cls[sorted_idx]
            if(assoc_ids is not None):
                assoc_ids = assoc_ids[sorted_idx]
        else:
            masks = polygons2masks((h, w), segments, color=1, downsample_ratio=self.mask_ratio)

        if(assoc_ids is None):
            return masks, instances, cls
        else:
            return masks, instances, cls, assoc_ids
